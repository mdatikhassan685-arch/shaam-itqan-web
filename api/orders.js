...
export default async function handler(req, res) {
    ...
    try {
        if (req.method === 'GET') {
            ...
            const b = req.body;
            // এখানে ৮টি কলাম: customer_name, phone, address, email, products, total_price, status, image_url
            await db.execute(
                'INSERT INTO orders (customer_name, phone, address, email, products, total_price, status, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                [b.name || 'N/A', b.phone || 'N/A', b.address || 'N/A', b.email || 'guest', b.products, b.total_price, b.status || 'Checkout_Pending', b.image_url || '']
            );
